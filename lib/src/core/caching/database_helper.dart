...
class DatabaseHelper {
  ...
  static Future<void> init() async {
    ...
    if(storedKey != null) {
      ...
      await secureStorage.write(key: 'encryptionKey', value: base64UrlEncode(cipherKey));
    }
  ...
  static Future<void> putItem({required String boxName,required String key, required dynamic item}) async {
  ...
  static Future<void> deleteItem({required String boxName,required int key}) async {
  ...
  static T? getItemAsObject<T>({required String boxName,required String key,required T Function(dynamic json) fromJson}) {
    final item = Hive.box(boxName).get(key);
    if(item is String){
